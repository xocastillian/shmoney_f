...
import type { Wallet, WalletBalanceSummary, WalletStatus } from '@/types/entities/wallet'
...
interface WalletsState {
	...
	updateWalletStatus: (walletId: number, status: WalletStatus) => void
...
export const useWalletsStore = create<WalletsState>(set => ({
	...
	updateWalletStatus: (walletId, status) =>
		set(state => ({
			wallets: state.wallets.map(w => (w.id === walletId ? { ...w, status } : w)),
		})),
